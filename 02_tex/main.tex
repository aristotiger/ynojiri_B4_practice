...
\author{知能機械工学科 中村(友)研究室\\123456 中村友昭} % 所属，学籍番号，名前
\date{}
...
``main.pdf''と``main.tex''には練習という項目があるので，指示に従い編集しながら練習してください．
TeXの使い方がある程度分かったら，\ref{sec:must_kadai}章の課題に取り組んでください．
...
コンパイルは左上の再生ボタン（っぽい形のボタン）, Mac OSは「タイプセット」を押すとコンパイルされて，PDFが作成されます．
...
\footnotetext[3]{上部で$\backslash$input\{mathdef\}を記述する必要がある} 
...
%%%%%%%%%%%%%%% ここまで %%%%%%%%%%%%%%%%%
\section{課題}
\label{sec:must_kadai}
``main.tex''と同じフォルダにある``kadai.tex"を書き換えて， 自己紹介シートを作成しましょう． 
詳しい説明は``kadai.tex" or ``kadai.pdf" に記載してあります．
この課題は提出が必要な課題となるので，指示された期限内に指定された方法で提出するようにしてください．


%\section{Extra資料}
%上の章までを出来るようになれば，B4の夏季に提出しなければならない集中輪講を乗り切れるので頑張りましょう．
%
%また，今後，研究の成果を論文にする機会があるかと思います．
%しかし，厄介なことに学会ごとに投稿する論文の見た目(フォーマット)が異なります．
%そこで，学会のフォーマットに簡単に合わせるためのファイル(``*.sty'', ``*.bst'')の使い方を``EX-kadai/texprac\_exmission.pdf''，``EX-kadai/texprac\_exmission.tex"に記載してあります．
%この資料では，慣れる意味も込めて学会指定のフォーマットを使った自己紹介シートの作成課題を用意しています．
%ひとまずは\ref{sec:must_kadai}章の課題が出来れば問題ないので，こちらは\red{提出不要}です．
